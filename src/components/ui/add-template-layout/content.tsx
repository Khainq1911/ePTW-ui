...
import { createTemplateService, updateTemplateService } from '../../../services/templates.service';
...
import TemplatePreview from '../template-form';

enum PATH {
    CREATE = '/template/add',
}

export default function TemplateContent({ id, items, dispatch, state, updateId }: any) {
    ...
    return (
        <div className="ml-[300px] bg-[#F5F6F7] p-4 flex h-full overflow-scroll flex-col">
            <div className="flex justify-between mb-8">
                ...
                <Button className="float-left" color="success" variant="contained" onClick={handleClickOpen}>
                    Save
                </Button>
            </div>

            <div className={`w-[800px] mx-auto bg-white rounded ${tab === 'preview' ? '' : 'hidden'}`}>
                <TemplatePreview template={state} />
            </div>

            <div className={`w-[800px] mx-auto bg-white p-4 rounded ${tab === 'edit' ? '' : 'hidden'}`}>
                                ...
                                <Draggable key={item.id} draggableId={item.id} index={index}>
                                    {(provided) => (
                                        ...
                                        >
                                            ...
                                            {renderItem(item.type, item.label, item.id, item?.options, dispatch)}
                                        </div>
                                    )}
                                </Draggable>
                            ))}
