...
import PermitInformation from '../../permit/permitInformation';
...
export default function TemplatePreview({ item, userName, dispatch, state }: PreviewProps) {
        ...
        <div className="h-full shadow bg-white p-10 rounded-xl grid gap-8">
            <div className="flex justify-between items-center">
                ...
                <FormControl sx={{ m: 1, minWidth: 200, marginBottom: '30px' }}>
                    ...
                    <Select
                        ...
                        id="receiver"
                        label="Receiver"
                        value={state?.receiverId || state?.receiver?.id || ''}
            ...
            <div className="flex justify-between items-center">
                ...
                <div className="flex justify-center items-center gap-2">
                    <div>
                        <p className="text-[16px] text-end">{userName || '[User Full Name]'}</p>
                        <p className="text-gray-400 text-[14px] text-end">
                            Created
                            <span className="ml-1">{formatDate(item?.created_at, 'dd, MM, DD, YYYY HH:MM A')}</span>
                        </p>
                    </div>
            ...
            <div className="grid gap-8">
                {(state ? state?.data : item?.fields)?.map((field: any, index: number) => (
